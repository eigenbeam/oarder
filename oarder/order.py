...
def oarder(order_request):
    ...
    spatial = BBox(**order_request.spatial_bbox)
    start_dt = dt.datetime.strptime(order_request.start_datetime, "%m/%d/%yT%H:%M:%SZ")
    stop_dt = dt.datetime.strptime(order_request.end_datetime, "%m/%d/%yT%H:%M:%SZ")
    temporal = {"start": start_dt, "stop": stop_dt}
